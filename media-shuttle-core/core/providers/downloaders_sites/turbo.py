...
from ..parsers_sites.turbo import resolve_turbo_source
from ..user_agents import with_random_user_agent
from .common import download_live_generic


def download_turbo_live(source: ParsedSource) -> DownloadResult:
    refreshed = _turbo_refresh_source(source)
    ...
    retried = _turbo_refresh_source(source, force=True)
    return download_live_generic(retried, headers=headers, actual_url=retried.download_url)


def _turbo_refresh_source(source: ParsedSource, force: bool = False) -> ParsedSource:
    if not force and _looks_like_turbo_direct_url(source.download_url):
        return source

    refreshed = resolve_turbo_source(source)
    if not refreshed:
        raise RuntimeError("failed to refresh turbo download url")
...
def _looks_like_turbo_direct_url(url: str) -> bool:
    return ".turbocdn.st/data/" in url
